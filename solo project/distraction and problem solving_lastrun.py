...
expName = 'distraction and problem solving'  # from the Builder filename that created this script
...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='C:\\Users\\NCS\\Desktop\\NIRJA\\lab in psych\\solo project\\distraction and problem solving_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    # create speaker 'notif_sound'
    deviceManager.addDevice(
        deviceName='notif_sound',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    text_4 = visual.TextStim(win=win, name='text_4',
        text='You will be shown some simple arithmetic problem, for example, 3+5-6. You have to report the answer in the text box provided.\n\nRound 1',
        font='Arial',
    ...
    problem = visual.TextStim(win=win, name='problem',
        text='',
        font='Arial',
    ...
    button_2 = visual.ButtonStim(win, 
        text='Click to submit', font='Arvo',
        pos=(0, -0.3),
        letterHeight=0.05,
    ...
    notif_polygon = visual.Rect(
        win=win, name='notif_polygon',
        width=(0.5, 0.2)[0], height=(0.5, 0.2)[1],
        ori=0.0, pos=(0.5, 0), anchor='center',
        lineWidth=1.0,     colorSpace='rgb',  lineColor=[-1.0000, -1.0000, -1.0000], fillColor=[-1.0000, -1.0000, -1.0000],
        opacity=1.0, depth=-3.0, interpolate=True)
    notif_sound = sound.Sound(
        'A', 
        secs=0.8, 
        stereo=True, 
        hamming=True, 
        speaker='notif_sound',    name='notif_sound'
    )
    notif_sound.setVolume(1.0)
    ...
    for thisTrial in trials:
        ...
        button_2.reset()
        # keep track of which components have finished
        trialComponents = [problem, textbox, button_2]
    ...
    continueRoutine = True
    # update component parameters for each repeat
    thisExp.addData('round_2.started', globalClock.getTime(format='float'))
    # keep track of which components have finished
    round_2Components = [instruction_2]
    ...
    while continueRoutine and routineTimer.getTime() < 1.0:
        ...
        # if instruction_2 is stopping this frame...
        if instruction_2.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > instruction_2.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                instruction_2.tStop = t  # not accounting for scr refresh
                instruction_2.tStopRefresh = tThisFlipGlobal  # on global time
                instruction_2.frameNStop = frameN  # exact frame index
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'instruction_2.stopped')
                # update status
                instruction_2.status = FINISHED
    ...
    for thisComponent in round_2Components:
        if hasattr(thisComponent, "setAutoDraw"):
            thisComponent.setAutoDraw(False)
    thisExp.addData('round_2.stopped', globalClock.getTime(format='float'))
    # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
    if routineForceEnded:
        routineTimer.reset()
    else:
    ...
    trials_2 = data.TrialHandler(nReps=1.0, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('conditions file 2.xlsx'),
    ...
    for thisTrial_2 in trials_2:
        ...
        text_3.setText(test_problems)
        textbox_3.reset()
        # reset button to account for continued clicks & clear times on/off
        button.reset()
        notif_polygon.setOpacity(opacity)
        notif_sound.setSound('500', secs=0.8, hamming=True)
        notif_sound.setVolume(int(intensity), log=False)
        notif_sound.seek(0)
        # keep track of which components have finished
        distractionComponents = [text_3, textbox_3, button, notif_polygon, notif_sound]
        ...
        while continueRoutine:
            ...
            # *notif_polygon* updates
            
            # if notif_polygon is starting this frame...
            if notif_polygon.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                notif_polygon.frameNStart = frameN  # exact frame index
                notif_polygon.tStart = t  # local t and not account for scr refresh
                notif_polygon.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(notif_polygon, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'notif_polygon.started')
                # update status
                notif_polygon.status = STARTED
                notif_polygon.setAutoDraw(True)
            
            # if notif_polygon is active this frame...
            if notif_polygon.status == STARTED:
                # update params
                pass
            
            # if notif_polygon is stopping this frame...
            if notif_polygon.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > notif_polygon.tStartRefresh + 0.8-frameTolerance:
                    # keep track of stop time/frame for later
                    notif_polygon.tStop = t  # not accounting for scr refresh
                    notif_polygon.tStopRefresh = tThisFlipGlobal  # on global time
                    notif_polygon.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'notif_polygon.stopped')
                    # update status
                    notif_polygon.status = FINISHED
                    notif_polygon.setAutoDraw(False)
            
            # if notif_sound is starting this frame...
            if notif_sound.status == NOT_STARTED and tThisFlip >= 0.5-frameTolerance:
                # keep track of start time/frame for later
                notif_sound.frameNStart = frameN  # exact frame index
                notif_sound.tStart = t  # local t and not account for scr refresh
                notif_sound.tStartRefresh = tThisFlipGlobal  # on global time
                # add timestamp to datafile
                thisExp.addData('notif_sound.started', tThisFlipGlobal)
                # update status
                notif_sound.status = STARTED
                notif_sound.play(when=win)  # sync with win flip
            
            # if notif_sound is stopping this frame...
            if notif_sound.status == STARTED:
                # is it time to stop? (based on global clock, using actual start)
                if tThisFlipGlobal > notif_sound.tStartRefresh + 0.8-frameTolerance:
                    # keep track of stop time/frame for later
                    notif_sound.tStop = t  # not accounting for scr refresh
                    notif_sound.tStopRefresh = tThisFlipGlobal  # on global time
                    notif_sound.frameNStop = frameN  # exact frame index
                    # add timestamp to datafile
                    thisExp.timestampOnFlip(win, 'notif_sound.stopped')
                    # update status
                    notif_sound.status = FINISHED
                    notif_sound.stop()
            # update notif_sound status according to whether it's playing
            if notif_sound.isPlaying:
                notif_sound.status = STARTED
            elif notif_sound.isFinished:
                notif_sound.status = FINISHED
        ...
        notif_sound.pause()  # ensure sound has stopped at end of Routine
